 ...
 *     Param::ShuntType=5. ControlContactors() is called every 10ms from
 * Ms10Task with the current opmode (0=OFF, 1=RUN, 2=PRECHARGE, 3=PCHFAIL,
 * 4=CHARGE). It drives the contactors via 0x10B at 20ms with CRC + alive
 * counter.
 *
 * ============================================================================
 * CAN BUS SETUP
 ...
 *               byte2: service request, emergency error, DCSW error, iso
 * warning byte3: cold shutoff valve byte6: min temperature (-50 offset, °C)
 ...
 * - Contactor close CONFIRMATION is measured, not assumed:
 * AreContactorsClosed() only reports true once post-contactor voltage (UDS
 * 0xDD66, polled every fast slot while waiting) tracks pack voltage within 10%,
 * or pack current >5A is flowing. Until then udc stays 0 and the VCU remains in
 * precharge, so an SME that refuses to close leads to PCHFAIL instead of a
 * false RUN.
...
const uint8_t BmwPhevBMS::UDS_VOLTAGE_LIMITS[5] = {0x07, 0x03, 0x22, 0xDD,
                                                   0x7E};
const uint8_t BmwPhevBMS::UDS_CURRENT_LIMITS[5] = {0x07, 0x03, 0x22, 0xDD,
                                                   0x7D};
...
const uint8_t BmwPhevBMS::UDS_BALANCING_STATUS[8] = {0x07, 0x04, 0x31, 0x03,
                                                     0xAD, 0x6B, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_READ_DTC[8] = {0x07, 0x03, 0x19, 0x02,
                                             0x0C, 0x00, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_CLEAR_DTC[8] = {0x07, 0x04, 0x14, 0xFF,
                                              0xFF, 0xFF, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_HARD_RESET[4] = {0x07, 0x02, 0x11, 0x01};
const uint8_t BmwPhevBMS::UDS_BALANCING_START[8] = {0x07, 0x04, 0x31, 0x01,
                                                    0xAD, 0x6B, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_BALANCING_STOP[8] = {0x07, 0x04, 0x31, 0x02,
                                                   0xAD, 0x6B, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_ISOLATION_TEST[8] = {0x07, 0x04, 0x31, 0x01,
                                                   0xAD, 0x61, 0x00, 0x00};
const uint8_t BmwPhevBMS::UDS_ISOLATION_STATUS[8] = {0x07, 0x04, 0x31, 0x03,
                                                     0xAD, 0x61, 0x00, 0x00};
 ...
 * UDS Poll Arrays
 *===========================================================================*/
const uint8_t *BmwPhevBMS::fastReqs[4] = {UDS_CELLSUMMARY, UDS_SOC,
                                          UDS_VOLTAGE_LIMITS, UDS_POST_VOLTAGE};
const int BmwPhevBMS::numFastReqs = 4;

const uint8_t *BmwPhevBMS::slowReqs[9] = {
    UDS_ISO_READING1,    UDS_ISO_READING2, UDS_CURRENT_LIMITS,   UDS_SOH,
    UDS_CELL_VOLTAGES,   UDS_CELL_TEMP,    UDS_BALANCING_STATUS, UDS_READ_DTC,
    UDS_ISOLATION_STATUS};
const int BmwPhevBMS::numSlowReqs = 9;

/*===========================================================================
 * Vehicle Environment CAN Frames (static, keep SME happy)
 *===========================================================================*/
const uint8_t BmwPhevBMS::VEH_3A0[8] = {0xFF, 0xFF, 0xCF, 0xFF,
                                        0xFF, 0xFF, 0xFF, 0xFD};
const uint8_t BmwPhevBMS::VEH_3CA[8] = {0xB7, 0x60, 0x01, 0x0F,
                                        0x0F, 0x31, 0xFF, 0xFF};
 ...
 * CRC / Alive Helpers
 *===========================================================================*/
uint8_t BmwPhevBMS::Crc8SAEJ1850(const uint8_t *data, uint8_t len,
                                 uint8_t init) {
  uint8_t crc = init;
...
void BmwPhevBMS::OnUdsResponse(uint8_t moduleID, const uint8_t *data,
  ...
  case 0x61: // Isolation measurement routine status (71 03 AD 61 ss ff)
    ...
    if (len >= 5 && data[0] == 0x71 && data[1] == 0x03 && data[2] == 0xAD &&
        data[3] == 0x61) {
  ...
  case 0x66: // Post-contactor voltage (62 DD 66), raw in cV (0.01 V/bit —
    ...
    if (len >= 5) {
      ...
      if (contactorCloseReq && packVoltage_dV > 100) {
        int32_t delta =
            (int32_t)packVoltage_dV - (int32_t)postContactorVoltage_dV;
        if (delta < 0)
          delta = -delta;
  ...
  case 0x02: // DTC response (SF 59 02 FF = none stored, or MF 59 02 FF +
             // records)
      ...
      for (uint16_t off = 3; off + 3 < len; off += 4) {
        ...
        if (stored < 5) {
          ...
          for (uint8_t i = 0; i < 4; i++)
            dtcCodes[i] = dtcCodes[i + 1];
          dtcCodes[4] = code;
        }
      }
      for (uint8_t i = stored; i < 5; i++)
        dtcCodes[i] = 0;
      dtcCount = total;
 ...
 *===========================================================================*/
static void SendCAN(CanHardware *can, uint32_t id, const uint8_t *data,
                    uint8_t dlc) {
  if (!can)
    return;
  uint8_t buf[8] = {0};
...
void BmwPhevBMS::DecodeCAN(int id, uint8_t *data) {
  ...
  case 0x112: // [20ms] Status Of High-Voltage Battery 2
    ...
    packCurrent_dA =
        (int32_t)(((data[1] << 8) | data[0]) - 8192); // 0.1A/bit, offset 8192
    ...
    {
      uint8_t openReq =
          (data[5] & 0xC0) >> 6; // battery_request_open_contactors
      uint8_t openReqInst =
          (data[6] & 0x03); // battery_request_open_contactors_instantly
      uint8_t openReqFast =
          (data[6] & 0x0C) >> 2; // battery_request_open_contactors_fast
      if (openReq == 2 || openReqInst == 2 || openReqFast == 2)
        smeDemandsOpen = true; // SME wants contactors open — safe shutdown in
                               // ControlContactors
      // "Signal invalid" (11) on any field: don't trip a shutdown, but
      // stop charging until the SME reports valid data again.
      smeSignalsInvalid =
          (openReq == 3 || openReqInst == 3 || openReqFast == 3);
    }
    break;
...
float BmwPhevBMS::MaxChargeCurrent() {
  ...
  if (smeDemandsOpen || smeEmergencyLatched || smeSignalsInvalid)
    return 0;
  ...
  if (packLimitAvailable && packVoltage_dV > maxDesignVoltage_dV)
    return 0;
  ...
  if (maxCellVoltage_mV > vmax * 1000.0f)
    return 0;
  if (minCellVoltage_mV < vmin * 1000.0f)
    return 0;
  if (maxTemperature_C > tmax)
    return 0;
  if (minTemperature_C < tmin)
    return 0;
...
void BmwPhevBMS::Task100Ms() {
  ...
  if (wakeInProgress) {
    ...
    if (bmsCan)
      bmsCan->SetBaudrate(CanHardware::Baud500);
  ...
  if (!wakeInProgress) {
    ...
    // ---- User-requested balancing on/off burst (driven by BMS_BalancingOn
    // param) ---- Edge detection each call: when the user toggles the web UI
    // param, fire a burst of START or STOP frames to latch the routine in the
    // SME.
    {
    ...
    if (balancingBurstRemaining > 0 && !udsOneShotPending) {
      ...
      SendCAN(bmsCan, 0x6F1,
              balancingBurstIsStart ? UDS_BALANCING_START : UDS_BALANCING_STOP,
              8);
    ...
    // Byte 7: maxTemperature_C (int8)
    {
      ...
      bool allowClose =
          contactorCloseReq && (startupCounter >= 320) &&
          (phev53aState == ST53A_ESCALATED || phev53aState == ST53A_STEADY) &&
          (preCloseStopsRemaining == 0) && !smeDemandsOpen &&
          !smeEmergencyLatched && !shutdownPending;
      uint8_t state = 0;
      if (AreContactorsClosed())
        state |= 0x80;
      if (contactorCloseReq)
        state |= 0x40;
      if (shutdownPending)
      ...
      if (uds.IsBusy())
        diag[6] = uds.GetModuleID(); // e.g. 0xA0=CELLSUMMARY, 0x7E=VoltLimits,
                                     // 0xC4=SOC
      else
    ...
    if ((tickCounter & 1) == 0 && tickCounter != 0 && !udsOneShotPending &&
        bmsCan) {
      ...
      if (!udsOneShotPending && bmsCan) {
        ...
        if (isoStatusBoostSecs > 0) {
          ...
          uint8_t dlc = (slowReqs[slowPollIndex] == UDS_BALANCING_STATUS ||
                         ...
                         slowReqs[slowPollIndex] == UDS_ISOLATION_STATUS)
                            ? 8
                            : 5;
          if (uds.SendRequest(slowReqs[slowPollIndex], dlc)) {
            slowPollIndex++;
            if (slowPollIndex >= numSlowReqs)
              slowPollIndex = 0;
          }
        }
      }
  ...
  if (minCellVoltage_mV != staleLastMinCell ||
      maxCellVoltage_mV != staleLastMaxCell) {
  ...
  if (timeoutCounter > 0) {
    ...
    Param::SetFloat(Param::BMS_Tavg,
                    (minTemperature_C + maxTemperature_C) / 2.0f);
    ...
    if (availDischargePowerLong_W > 0)
      Param::SetFloat(Param::BMS_MaxOutput,
                      availDischargePowerLong_W / 1000.0f);
    ...
    // Balancing status (UDS 0xAD6B: 0=inactive not needed, 1=active, 2=not
    // resting, 3=inactive, 4=unknown/qualifier invalid)
    ...
    // Pack power (V × A, W → kW)
    float packPower_kW =
        (packVoltage_dV / 10.0f) * (packCurrent_dA / 10.0f) / 1000.0f;
    ...
    Param::SetFloat(Param::BMS_CellDelta,
                    (float)(maxCellVoltage_mV - minCellVoltage_mV));
  } else {
...
void BmwPhevBMS::ControlContactors(int opmode, CanHardware *can) {
  ...
  if (requestClose && !contactorCloseReq && !smeEmergencyLatched) {
    ...
    // Request open — enter safe shutdown: wait for current to decay before
    // opening
  ...
  ms10Counter++;
  if (ms10Counter >= 2 &&
      !wakeInProgress) // no TX while bus is at 100kbps for wakeup
  {
    ms10Counter = 0;
...
bool BmwPhevBMS::AreContactorsClosed() {
  return contactorCloseReq && phev53aState == ST53A_STEADY &&
