 ...
 *            control via 0x10B/0x53A, vehicle environment CAN frames, SME
 * wakeup.
 *
 * The SME communicates via ISO-TP (UDS over CAN, extended addressing, ID
 * 0x6F1→0x607). Contactors are driven by 0x10B (CRC + alive counter) with 0x53A
 * providing the associated vehicle/contactor STATE indication (A/B/C/D
 * sequence).
 *
...
private:
  ...
  static void OnUdsResponse(uint8_t moduleID, const uint8_t *data, uint16_t len,
                            void *ctx);
  ...
  static bool smeEmergencyLatched; // set after an SME-demanded open; blocks
                                   // re-close until opmode leaves RUN/CHARGE
  static bool smeSignalsInvalid;   // true while any 0x112 open-request field
                                   // reads "signal invalid"
  static bool
      shutdownPending; // true when MOD_OFF requested, waiting for current decay
  static uint8_t smeOpenDelayTicks; // countdown after current drops before
                                    // forcing open (shared)

  // ---- Contactor close confirmation (measured feedback) ----
  static bool contactorsConfirmedClosed; // post-contactor V tracks pack V, or
                                         // current flowing
  ...
  static uint16_t timeoutCounter; // decremented each Task100Ms, reset on data
                                  // rx (BMS_Timeout*10, up to 1200)
  static bool batteryAwake;
  static bool wakeInProgress; // bus is at 100kbps for SME wakeup — suppress all
                              // other TX
  ...
  static int32_t packCurrent_dA;  // from 0x112 (signed, deci-amps: raw 0.1A/bit
                                  // minus 8192 offset)
  static uint16_t postContactorVoltage_dV; // from UDS 0xDD66 (dV, used for
                                           // close confirmation)
  ...
  static uint16_t
      isoTrgKOhm; // from UDS 0xDD6A bytes 5-6 (trigger threshold reading)
  ...
  static uint8_t
      isoTestStatus; // 71 03 AD 61 byte5: 0=not run, 1=successful, 2=running
  static uint8_t
      isoTestFault; // 71 03 AD 61 byte6: 0=no fault, 1=fault, 0xFF=undefined
  static uint8_t
      isoStatusBoostSecs;   // 1Hz status polling countdown after test start
  ...
  static uint8_t
      interlockStat; // from 0x1FA byte1 bits0-1 (qualifier: 2 = not seated)
  static uint8_t
      prechargeStat; // from 0x1FA byte1 bits2-3 (2 = precharge blocked)
  static uint8_t
      dcswStat; // from 0x1FA byte1 bits4-5 (0=open 1=precharging 2=closed)
  static uint8_t
      emgModeStat; // from 0x1FA byte1 bits6-7 (qualifier: 2 = emergency)
  static uint8_t
      svcReqStat; // from 0x1FA byte2 bits0-1 (qualifier: 2 = service required)
  ...
  static CanHardware
      *bmsCan; // CAN interface for BMS UDS/data (set in SetCanInterface)
};
